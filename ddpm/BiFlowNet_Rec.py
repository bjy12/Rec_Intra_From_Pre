...
class BiFlowNet_Pre_Intra(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv3d(input_channels, init_dim, (init_kernel_size, init_kernel_size,
        ...
        self.patch_size = patch_size
        #* input_channels = noise feature channels + condition feature channels
        self.x_embedder = PatchEmbed_Voxel(sub_volume_size, patch_size, input_channels, dim, bias=True)
    ...
    def forward(
        ...
        if condition_dict is not None:
            # 使用你的 Condition Model 提取特征
            hint = self.condition_branch(condition_dict)
        else:
            # 异常处理：如果没有条件，可能需要一个零张量占位 (训练无条件模型时)
            # 但在你的任务中应该总是存在的
            raise ValueError("condition_dict cannot be None for conditional generation")  
        #pdb.set_trace()
        x_input = torch.cat((x , hint) , dim=1)          
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, denoise_fn,x, t, clip_denoised: bool, condition_dict=None, y=None, res=None,hint = None,cond_scale=1.):
        if hint == None:
            x_recon = self.predict_start_from_noise(
                        x, t=t, noise=denoise_fn(x, t, condition_dict=condition_dict, y=y,res=res))
        else:
            x_recon = self.predict_start_from_noise(
            x, t=t, noise=denoise_fn(x, t, condition_dict=condition_dict, y=y, res=res,hint = hint))
    ...
    def p_sample(self, denoise_fn , x, t, condition_dict=None, y=None, res= None,hint = None,cond_scale=1., clip_denoised=True):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(denoise_fn,
            x=x, t=t, clip_denoised=clip_denoised, condition_dict=condition_dict, y=y, res=res,hint = hint,cond_scale=cond_scale)
    ...
    @torch.inference_mode()
    def p_sample_loop(self, denoise_fn, z,  condition_dict=None, y=None, res=None,cond_scale=1.,hint = None):
        device = self.betas.device
        b = z.shape[0]
        img = default(z, lambda: torch.randn_like(z , device= device))
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(denoise_fn, img, torch.full(
                (b,), i, device=device, dtype=torch.long), condition_dict=condition_dict, y=y, res=res,cond_scale=cond_scale,hint= hint)
    ...
    def p_losses(self, denoise_fn,x_start, t, condition_dict=None,y=None, res=None,noise=None, hint = None,**kwargs):
        ...
        #     y = y.to(device)
        if hint == None:
            x_recon = denoise_fn(x_noisy, t, condition_dict=condition_dict)
        else:
